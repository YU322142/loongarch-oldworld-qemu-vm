...
- If ClassIsland renders correctly in this VM but shows large red or white blocks on real LoongArch/Kylin/Loongnix hardware, do not treat the VM result alone as a pass. QEMU usually has no reliable hardware GL path, while real hardware may use Avalonia GLX/EGL; use the newer ClassIsland LoongArch old-world package, which defaults to `CLASSISLAND_X11_RENDERING=software`. See the [testing checklist](docs/TESTING.md#avalonianet-application-checks).
