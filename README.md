...
- QEMU 启动过程中不要最大化窗口。实测在固件、GRUB、内核加载或桌面初始化阶段最大化可见窗口，可能导致 QEMU 或 guest 画面无响应。
- 桌面环境运行过程中也不建议最大化或拖动缩放 QEMU 窗口。实测窗口大小变化后可能出现鼠标点击位置偏移，影响按钮、菜单和托盘图标验收；请保持启动后的固定窗口大小完成测试。
- Loongnix X11 Test Desktop 是轻量测试会话，不提供完整桌面右键壁纸设置。本仓库自带 `shared\pic.png`，一键配置脚本会默认把它设为图片壁纸；壁纸有时需要把一个窗口完整拖过桌面区域触发重绘后才会显示。更多说明见 [完整使用教程](docs/USAGE.zh-CN.md#55-可选设置测试桌面壁纸)。
